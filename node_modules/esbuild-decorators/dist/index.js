...
const esbuildDecorators = (options = {}) => ({
    ...
    setup(build) {
        ...
        const tsconfigPath = options.tsconfig || ((_a = build.initialOptions) === null || _a === void 0 ? void 0 : _a.tsconfig) || path.join(cwd, './tsconfig.json');
        ...
        build.onLoad({ filter: tsx ? /\.tsx?$/ : /\.ts$/ }, async (args) => {
            ...
            const ts = await promises.readFile(args.path, 'utf8').catch((err) => printDiagnostics({ file: args.path, err }));
            if (!ts) {
                return;
            }
